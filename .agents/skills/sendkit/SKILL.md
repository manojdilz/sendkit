---
name: sendkit
description: Send Telegram messages from an agent through the SendKit MCP `telegram` tool, with the SendKit CLI (`@manoj_dilz/sendkit`) as a fallback. Use when a user asks to send a Telegram message, mentions SendKit, wants to interact with the SendKit toolset, asks to verify SendKit manually, or needs to choose between the SendKit MCP and CLI workflows.
---

# SendKit

SendKit sends Telegram messages. It exposes the same operation two ways, both backed by `@manoj_dilz/sendkit-core`:

- **MCP tool** (`sendkit` server → `telegram` tool) — preferred for agents.
- **CLI** (`@manoj_dilz/sendkit`, binary `npx sendkit`) — fallback when MCP is unavailable or for manual verification.
...
**Always invoke the CLI as `npx sendkit ...`** — never use the scoped package form (`npx @manoj_dilz/sendkit ...`) or `bunx` variants. `npx sendkit` is the only correct command for this workflow.

First-time setup writes a token to `~/.config/sendkit/config.json` (mode `0600`):

```bash
npx sendkit init --telegram-bot-token <botToken>
```

Send a message:

```bash
npx sendkit telegram <chatId> <message>
```
...
```bash
npx sendkit telegram <yourChatId> "SendKit test message"
```
